...
function testBrowser() {
    ...
    if (iAaPos > -1) {
        if (parseInt(sBrowser.charAt(iAaPos + 5)) > 5) {
            return true
        }
    }
    if (iAaOpera > -1) {
        if (parseInt(sBrowser.charAt(iAaOpera + 6)) > 8) {
            return true
        }
    }
    return false;
...
function validateRadio(theform, form_obj) {
    var radios = theform.elements[form_obj.name];
...
function validateForm(theform, msg) {
    ...
    for (x = 0; x <= form_elements.length - 1; x++) {
        ...
        if (sClassName != undefined && sClassName.indexOf('required') > -1) {
            ...
            if (sType == 'checkbox' && form_obj.checked != 1) {
                sValue = ''
            }
            ;
            if (sType == 'radio') {
                sValue = validateRadio(theform, form_obj)
            }
            ;
...
function opacityGet(obj) {
    ...
    if (val == '') {
        val = obj.style.MozOpacity
    }
    if (val == '') {
        val = obj.style.KhtmlOpacity
    }
    if (isNaN(val)) {
        val = 100
    } else {
...
function fader(idx) {
    ...
    if (Math.round(valx) != aFadeObjs[idx].tgt) {
        ...
        aFadeObjs[idx].tmr = setTimeout('fader(' + idx + ')', 25);
    } else {
...
function fadeTo(objx, opacity) {
    if (fadeTgtVal(objx) != opacity) {
        ...
        if (secs < 0.1) {
            opacitySet(objx, opacity)
        } else {
            var opacityx = opacityGet(objx);
            if (opacityx != opacity) {
                var stepx = (opacity - opacityx) / (secs * 40);
                if (idx < 0) {
                    idx = aFadeObjs.length;
                    aFadeObjs[idx] = {origo: opacityx}
                }
                ;var tmrx = setTimeout('fader(' + idx + ')', 25);
...
function slider(idx) {
    ...
    if (Math.round(valx) != aSlideObjs[idx].tgtx || Math.round(valy) != aSlideObjs[idx].tgty) {
        objx.style.left = valx + 'px';
        objx.style.top = valy + 'px';
        aSlideObjs[idx].tmr = setTimeout('slider(' + idx + ')', 20);
    } else {
        objx.style.left = aSlideObjs[idx].tgtx + 'px';
...
function slideTo(objx, xpos, ypos, secs) {
    ...
    var sx = (xpos - valx) / (secs * 50);
    var sy = (ypos - valy) / (secs * 50);
    if (idx < 0) {
        idx = aSlideObjs.length;
        var origx = valx;
        var origy = valy
    } else {
        var origx = aSlideObjs[idx].ox;
        var origy = aSlideObjs[idx].oy
    }
    ;var tmrx = setTimeout('slider(' + idx + ')', 20);
    aSlideObjs[idx] = {
        obj: objx.id,
        tmr: tmrx,
        x: valx,
...
function resizeHalt(obj) {
    var id = obj.id;
    for (x = 0; x < aResizeObjs.length; x++) {
        if (aResizeObjs[x].obj == id) {
            clearTimeout(aResizeObjs[x].tmr);
            return x;
            break;
        }
    }
...
function resizer(idx) {
    var objx = document.getElementById(aResizeObjs[idx].obj);
    var valw = Number(aResizeObjs[idx].w) + Number(aResizeObjs[idx].stepw);
    aResizeObjs[idx].w = valw;
    var valh = Number(aResizeObjs[idx].h) + Number(aResizeObjs[idx].steph);
    aResizeObjs[idx].h = valh;
    if (Math.round(valw) != aResizeObjs[idx].tgtw || Math.round(valh) != aResizeObjs[idx].tgth) {
        objx.style.width = valw + 'px';
        objx.style.height = valh + 'px';
        aResizeObjs[idx].tmr = setTimeout('resizer(' + idx + ')', 20);
    } else {
        objx.style.width = aResizeObjs[idx].tgtw + 'px';
        objx.style.height = aResizeObjs[idx].tgth + 'px';
    }
}

function sizeTo(objx, wpos, hpos, secs) {
    var idx = resizeHalt(objx);
    var valw = stripPx(objx.style.width);
    var valh = stripPx(objx.style.height);
    var sw = (wpos - valw) / (secs * 50);
    var sh = (hpos - valh) / (secs * 50);
    if (idx < 0) {
        idx = aResizeObjs.length;
        var origw = valw;
        var origh = valh
    } else {
        var origw = aResizeObjs[idx].ow;
        var origh = aResizeObjs[idx].oh
    }
    ;var tmrx = setTimeout('resizer(' + idx + ')', 20);
    aResizeObjs[idx] = {
        obj: objx.id,
        tmr: tmrx,
        w: valw,
...
function zoomTo(objx, factor, secs) {
    var wpos;
    var hpos;
    slideHalt(objx);
    var idx = resizeHalt(objx);
    if (idx < 0) {
        wpos = stripPx(objx.style.width);
        hpos = stripPx(objx.style.height)
    } else {
        wpos = (aResizeObjs[idx].ow);
        hpos = (aResizeObjs[idx].oh)
    }
    var wtgt = Math.round(wpos * factor);
    var htgt = Math.round(hpos * factor);
    var xtgt = Math.round(stripPx(objx.style.left) - ((wtgt - stripPx(objx.style.width)) / 2));
    var ytgt = Math.round(stripPx(objx.style.top) - ((htgt - stripPx(objx.style.height)) / 2));
    sizeTo(objx, wtgt, htgt, secs);
    slideTo(objx, xtgt, ytgt, secs);
}

function restorePosSize(objx, secs) {
    var idx = slideHalt(objx);
    if (idx > -1) {
        var x = aSlideObjs[idx].ox;
        var y = aSlideObjs[idx].oy;
        slideTo(objx, x, y, secs);
    }
    idx = resizeHalt(objx);
    if (idx > -1) {
        var w = aResizeObjs[idx].ow;
        var h = aResizeObjs[idx].oh;
...
function rollFind(id) {
    for (x = 0; x < aRollObjs.length; x++) {
        if (aRollObjs[x].obj == id) {
            return x;
            break;
        }
    }
...
function rollInit(objx, objx1, objx3, fOpacity, fTime, fHover, fDown, fSwap, sDefault) {
    ...
    if (fDown != '') {
        var b2 = 0;
        iDown = new Image();
        iDown.onerror = function (evt) {
            if (b2 == 0) {
                b2 = 1;
                iDown.src = iHover.src;
            }
            ;
        };
        iDown.src = fDown;
...
function loadFile(sUrl) {
    var vRequest = new ajaxRequest();
    vRequest.open('GET', sUrl, false);
    vRequest.setRequestHeader('User-Agent', navigator.userAgent);
    if (vRequest.overrideMimeType) {
        vRequest.overrideMimeType('text/plain');
    }
    vRequest.send(null);
    if (vRequest.readyState == 4) {
        return vRequest.responseText;
    }
}
...
function butnInit(objx, objx3, fOpacity, fTime, fSwap) {
    ...
    var sStatesFile = sSrc.substr(0, i + 1) + 'states.txt';
    var sStates = loadFile(sStatesFile);
    i = sSrc.lastIndexOf('.');
    var sExt = sSrc.substr(i, 4);
    i = sSrc.indexOf('_up.');
    sSrc = sSrc.substr(0, i);
    if (sStates.indexOf('[h]') > -1) {
        fHover = sSrc + '_hover' + sExt;
    } else {
        fHover = sSrc + '_up' + sExt;
    }
    if (sStates.indexOf('[d]') > -1) {
        fDown = sSrc + '_down' + sExt;
    } else {
        fDown = sSrc + '_up' + sExt;
    }
...
function rollOver(id) {
    ...
    ;var aItem = aRollObjs[idx];
    var objx;
    if (aItem.img1 != -1) {
        objx = document.getElementById(aItem.obj1);
        opacitySet(objx, 0);
        objx.src = aItem.img1.src;
        objx.style.visibility = 'visible';
    } else {
        objx = document.getElementById(id);
        if (aItem.op1 == -1) {
            aItem.op1 = opacityGet(objx)
        }
        ;
    }
    ;
    if (aItem.obj3 != '' && aItem.img3 != -1) {
        document.getElementById(aItem.obj3).src = aItem.img3.src;
    }
    ;fadeTo(objx, aItem.op2, aItem.tm);
}
...
function rollDown(id) {
    ...
    ;
    if (aRollObjs[idx].img2 != -1) {
        objx = document.getElementById(aRollObjs[idx].obj1);
        objx.src = aRollObjs[idx].img2.src;
        fadeTo(objx, aRollObjs[idx].op2);
    }
}
...
function rollOut(id) {
    ...
    if (aRollObjs[idx].img1 != -1 || aRollObjs[idx].img2 != -1) {
        ...
        if (aRollObjs[idx].img1 != -1) {
            objx.src = aRollObjs[idx].img1.src;
        } else {
            fadeTo(objx, 0, 0);
        }
        ;
...
function getPostData(theForm) {
    ...
    for (i = 0; i < theForm.elements.length; i++) {
        ...
        switch (obj.type) {
            ...
            case 'password':
            case 'textarea':
                sPostData += obj.name + '=' + escape(obj.value) + '&';
                break;
        }
    }
    return sPostData;
...
function ajaxPostForm(sPageURL, theForm, fCallBack) {
    ajaxPostQuery(sPageURL, getPostData(theForm), fCallBack);
}
...
function ajaxPostQuery(sPageURL, sPostData, fCallBack) {
    ...
    ajaxReq.open('POST', sPageURL, true);
    ajaxReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    ajaxReq.onreadystatechange = function () {
        if (ajaxReq.readyState == 4) {
            eval(fCallBack + '(ajaxReq.responseText);');
        }
    };
...
function getHttpField(sField, sPageURL, fCallBack) {
    ...
    if (ajaxReq.overrideMimeType) {
        ajaxReq.overrideMimeType('text/plain');
    }
    ajaxReq.open('GET', sPageURL, true);
    ajaxReq.onreadystatechange = function () {
        if (ajaxReq.readyState == 4) {
            eval(fCallBack + '(ajaxReq.getResponseHeader(sField));');
        }
    };
    ajaxReq.send(null);
}
...
function getWindowSize() {
    if (window.innerWidth) {
        var pt = {width: window.innerWidth, height: window.innerHeight};
    } else {
        var pt = {width: document.body.clientWidth, height: document.body.clientHeight};
    }
    return pt;
}
...
function fixedIsSupported() {
    ...
    var result = (obj.offsetTop == 1);
    document.body.removeChild(obj);
    return result;
}

var sysWinDimCallback;
var sysWinDimTime;
var sysWinDimBusy = 0;

function fRes() {
    var obj = document.getElementById('syswindim');
    obj.style.height = '1px';
    obj.style.width = '1px';
...
function windowDim(opacity, time, color) {
    var argv = arguments;
    var argc = arguments.length;
    sysWinDimCallback = (argc > 3) ? argv[3] : function () {
    };
    sysWinDimTime = time;
    var obj = document.getElementById('syswindim');
    if (!obj) {
    ...
    if (!fixedIsSupported()) {
        fRes();
        window.attachEvent('onresize', fRes);
    } else {
        obj.style.position = 'fixed';
    ...
    fadeTo(obj, opacity, time);
}

function windowUndim() {
    if (!fixedIsSupported()) {
        window.detachEvent('onresize', fRes);
    }
    var obj = document.getElementById('syswindim');
    fadeTo(obj, 1, sysWinDimTime);
